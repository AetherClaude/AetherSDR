...
bool AudioEngine::startTxStream(const QHostAddress& radioAddress, quint16 radioPort)
{
    ...
    // Guard against silent stops (known Qt6/macOS CoreAudio behavior after extended runtime, #1149).
    // stateChanged may fire from a CoreAudio thread; QueuedConnection routes it back to this
    // thread's event loop before we touch any members.
    connect(m_audioSource, &QAudioSource::stateChanged, this,
            [this](QAudio::State state) {
        if (state != QAudio::StoppedState) return;
        // If m_txPollTimer is already null, stopTxStream() handled this intentionally.
        if (!m_txPollTimer) return;
        // Pause the timer immediately so onTxAudioReady() cannot fire against a stale
        // m_micBuffer, then schedule a clean restart on this thread's event loop.
        m_txPollTimer->stop();
...
{
#ifdef Q_OS_MAC
    if (!m_micBuffer || !m_audioSource || m_audioSource->state() == QAudio::StoppedState) return;
