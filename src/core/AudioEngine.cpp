...
bool AudioEngine::startRxStream()
{
    ...
    m_audioSink->setVolume(std::min(m_rxVolume.load(), 1.0f));
...
{
    m_rxVolume.store(qBound(0.0f, v, 2.0f));  // allow up to +6 dB software boost
    if (m_audioSink)
        m_audioSink->setVolume(m_muted.load() ? 0.0f : std::min(m_rxVolume.load(), 1.0f));
}
...
void AudioEngine::setMuted(bool muted)
{
    m_muted.store(muted);
    if (m_audioSink)
        m_audioSink->setVolume(muted ? 0.0f : std::min(m_rxVolume.load(), 1.0f));
}
...
void AudioEngine::feedAudioData(const QByteArray& pcm)
{
    ...
    auto writeAudio = [this](const QByteArray& data) {
        ...
        const QByteArray& boosted = (vol > 1.0f) ? applyBoost(data, vol) : data;
        if (m_resampleTo48k)
            m_rxBuffer.append(resampleStereo(boosted));
        else
