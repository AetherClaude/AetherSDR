            ...
            [this, udpPort](int code2, const QString&) {
                ...
                sendCmd("slice list",
                    [this](int code3, const QString& body) {
                        ...
                        // Always create the remote_audio_rx stream so that audio
                        // data flows to TCI clients (e.g. WSJT-X) even when local
                        // PC audio playback is disabled ("Radio Audio" mode, #1000).
                        // AudioEngine::feedAudioData() is a no-op when its sink is
                        // stopped, so no audio escapes to the local speaker in that
                        // case.
                        sendCmd(
