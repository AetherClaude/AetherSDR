            ...
            [this, udpPort](int code2, const QString&) {
                ...
                sendCmd("slice list",
                    [this](int code3, const QString& body) {
                        ...
                        // Request a remote audio RX stream.
                        // Create it when PC audio is enabled OR when TCI is enabled —
                        // TCI audio streaming also depends on this stream (#1014).
                        // When neither is active, audio plays through the radio's
                        // physical outputs (line out, headphone, front speaker).
                        {
                            auto& as = AppSettings::instance();
                            bool pcAudio = as.value("PcAudioEnabled", "True").toString() == "True";
                            bool tciActive = as.value("AutoStartTCI", "False").toString() == "True";
                            if (pcAudio || tciActive) {
                                sendCmd(
                                ...
                                qCDebug(lcProtocol) << "RadioModel: PC audio and TCI both disabled, skipping remote_audio_rx";
                            }
                        }
