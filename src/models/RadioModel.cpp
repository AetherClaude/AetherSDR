    ...
    sendCmd("display panafall create x=100 y=100", [this](int code, const QString& body) {
        ...
        if (!panId.isEmpty()) {
            QTimer::singleShot(200, this, [this, panId]() {
                ...
                const int fps = audioCatOnly ? 0 : 25;
                sendCmd(QString("display pan set %1 fps=%2 min_dbm=-130 max_dbm=-40")
                            .arg(panId).arg(fps));
            });
        }
    });
...
void RadioModel::registerAsGuiClient(const QString& clientId)
{
    ...
    sendCmd("sub slice all", [this](int, const QString&) {
      ...
      sendCmd("sub tx all", [this](int, const QString&) {
        ...
        // In audio+CAT only mode, zero FFT/waterfall bytes is expected. (#1088)
        QTimer::singleShot(10000, this, [this]() {
            if (!isConnected()) return;
            if (AppSettings::instance().value("AudioCatOnly", "False").toString() == "True") return;
...
void RadioModel::configurePan()
{
    ...
    // In audio+CAT only mode, set fps=0 to suppress all FFT streaming data.
    // The panadapter object must exist (API requirement for slice association)
    // but produces zero bandwidth when fps=0. (#1088)
    if (AppSettings::instance().value("AudioCatOnly", "False").toString() == "True") {
        sendCmd(QString("display pan set %1 fps=0").arg(m_activePanId));
        return;
    }
...
void RadioModel::configureWaterfall()
{
    ...
    // In audio+CAT only mode, skip waterfall configuration entirely — no
    // waterfall data is expected when fps=0 is set on the panadapter. (#1088)
    if (AppSettings::instance().value("AudioCatOnly", "False").toString() == "True")
        return;
