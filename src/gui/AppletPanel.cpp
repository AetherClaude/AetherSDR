...
public:
        ...
        : QWidget(parent), m_appletId(appletId), m_panel(panel)
    {
        ...
        m_grip = grip;
        m_grip->setStyleSheet(QString("QLabel { background: transparent; color: %1; font-size: 10px; }")
                              .arg(ThemePalette::current().dimText.name()));
        layout->addWidget(grip);

        m_label = new QLabel(text);
        m_label->setStyleSheet(QString("QLabel { background: transparent; color: %1; "
                           "font-size: 10px; font-weight: bold; }")
                           .arg(ThemePalette::current().headerText.name()));
...
protected:
    ...
    void updateStyle()
    {
        ...
        setStyleSheet(
            QString("QWidget { background: qlineargradient(x1:0,y1:0,x2:0,y2:1,"
                    "stop:0 %1, stop:0.5 %2, stop:1 %3); "
                    "border-bottom: 1px solid %4; }")
            .arg(pal.headerTop.name(), pal.headerMid.name(),
                 pal.headerBot.name(), pal.headerBorderBot.name()));
        if (m_grip)
            m_grip->setStyleSheet(QString("QLabel { background: transparent; color: %1; font-size: 10px; }")
                                  .arg(pal.dimText.name()));
        if (m_label)
            m_label->setStyleSheet(QString("QLabel { background: transparent; color: %1; "
                                           "font-size: 10px; font-weight: bold; }")
                                   .arg(pal.headerText.name()));
    }
...
void AppletPanel::refreshTheme()
{
    // Re-apply palette to all AppletTitleBar instances (#1160).
    // AppletTitleBar has no Q_OBJECT so we cast from the stored QWidget*;
    // all titleBar entries in m_appletOrder are always AppletTitleBar objects.
    for (auto& entry : m_appletOrder) {
        if (auto* tb = static_cast<AppletTitleBar*>(entry.titleBar))
            tb->updateStyle();
    }

    // Re-apply palette to combo boxes this panel owns
    for (auto* cb : findChildren<QComboBox*>())
        applyComboStyle(cb);

    // The SMeter repaints via QPainter so a repaint() is sufficient
    update();
}
