...
AppletPanel::AppletPanel(QWidget* parent) : QWidget(parent)
{
    ...
    // Write defaults on startup so keys are always present in settings (#809)
    if (!AppSettings::instance().contains("SMeter_TxSelect"))
        AppSettings::instance().setValue("SMeter_TxSelect", m_txSelect->currentIndex());
    if (!AppSettings::instance().contains("SMeter_RxSelect"))
        AppSettings::instance().setValue("SMeter_RxSelect", m_rxSelect->currentIndex());
