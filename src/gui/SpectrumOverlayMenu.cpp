...
void SpectrumOverlayMenu::buildDisplayPanel()
{
    ...
    // Line Width row (1–10 maps to 0.5–5.0 px in 0.5 steps)
    makeRow("Line Width:", 1, 10, 3, m_lineWidthSlider, m_lineWidthLabel);
    m_lineWidthLabel->setText("1.5");
    connect(m_lineWidthSlider, &QSlider::valueChanged, this, [this](int v) {
        const float w = v * 0.5f;
        m_lineWidthLabel->setText(QString::number(w, 'f', 1));
        emit fftLineWidthChanged(w);
    });
    m_lineWidthSlider->setToolTip("Thickness of the spectrum trace line (0.5–5.0 px).");
                                               ...
                                               bool showGrid, float lineWidth)
{
    ...
    if (m_lineWidthSlider) {
        ...
        const int sliderVal = std::clamp(static_cast<int>(lineWidth / 0.5f), 1, 10);
        m_lineWidthSlider->setValue(sliderVal);
        m_lineWidthLabel->setText(QString::number(sliderVal * 0.5f, 'f', 1));
    }
}
