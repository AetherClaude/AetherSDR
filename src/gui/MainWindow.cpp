    ...
    : QMainWindow(parent)
{
    ...
    connect(m_titleBar, &TitleBar::pcAudioToggled, this, [this](bool on) {
        // Toggle local playback only — keep the radio's remote_audio_rx stream
        // alive so TCI clients (e.g. WSJT-X) continue to receive audio
        // regardless of the PC Audio setting (#1000).
        if (on)
            audioStartRx();
        else
            audioStopRx();
...
void MainWindow::onConnectionStateChanged(bool connected)
{
    ...
    if (connected) {
        ...
        // Only start local playback sink if PC Audio is enabled (#1000).
...
void MainWindow::onSliceAdded(SliceModel* s)
{
    ...
    if (m_activeSliceId < 0) {
        ...
        // Always create so TCI clients receive audio even in Radio Audio mode (#1000).
