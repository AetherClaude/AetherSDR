...
void MainWindow::onConnectionStateChanged(bool connected)
{
    ...
    if (connected) {
        ...
        // In audio+CAT only mode, hide the spectrum display — no FFT data will arrive. (#1088)
        if (AppSettings::instance().value("AudioCatOnly", "False").toString() == "True")
            m_panStack->setVisible(false);
        ...
        // Restore spectrum panel visibility in case it was hidden for audio+CAT only mode. (#1088)
        m_panStack->setVisible(true);
...
{
    // In audio+CAT only mode, skip additional pan creation — no FFT data is expected. (#1088)
    if (AppSettings::instance().value("AudioCatOnly", "False").toString() == "True")
        return;
