    ...
    : QMainWindow(parent)
{
...
#ifdef HAVE_WEBSOCKETS
            // Keep the remote_audio_rx stream alive when TCI clients are
            // connected — TCI audio must work regardless of PC Audio state (#1014)
            if (m_tciServer && m_tciServer->clientCount() > 0)
                return;
#endif
    ...
    // Maintain the remote_audio_rx stream based on TCI client demand (#1014).
    // When a TCI client connects while Radio Audio (PC audio off) is active,
    // create the stream so TCI audio is available. When the last client
    // disconnects with PC audio off, release the stream.
