...
QWidget* RadioSetupDialog::buildAudioTab()
{
    ...
    // Route through QueuedConnection so setInputDevice/setOutputDevice execute
    // on the audio worker thread (not the GUI thread), preventing the
    // use-after-free / CoreAudio session conflict that caused the macOS crash
    // when switching devices (#1114).
    if (m_audio) {
