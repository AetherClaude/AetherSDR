      ...
      m_tgxl(tgxl), m_pgxl(pgxl), m_ag(ag)
{
    ...
    tabs->addTab(buildRotorTab(), "Rotor");
#ifdef HAVE_SERIALPORT
...
QWidget* RadioSetupDialog::buildRotorTab()
{
    auto* page = new QWidget;
    auto* vbox = new QVBoxLayout(page);
    vbox->setSpacing(8);
    ...
    // QTH group
    {
        auto* group = new QGroupBox("Operator QTH");
        group->setStyleSheet(kGroupStyle);
        auto* grid = new QGridLayout(group);
        grid->setSpacing(6);

        auto makeDblEdit = [](double val, double min, double max) {
            auto* e = new QLineEdit(QString::number(val, 'f', 4));
            e->setStyleSheet(
                "QLineEdit { background: #1a2a3a; border: 1px solid #304050; "
                "border-radius: 3px; color: #c8d8e8; font-size: 12px; padding: 2px 4px; }");
            e->setValidator(new QDoubleValidator(min, max, 6, e));
            return e;
        };

        grid->addWidget(new QLabel("Latitude (−90 to +90):"), 0, 0);
        auto* latEdit = makeDblEdit(s.value("RotorMyLat", 0.0).toDouble(), -90.0, 90.0);
        grid->addWidget(latEdit, 0, 1);
        for (auto* lbl : group->findChildren<QLabel*>())
            lbl->setStyleSheet(kLabelStyle);

        grid->addWidget(new QLabel("Longitude (−180 to +180):"), 1, 0);
        auto* lonEdit = makeDblEdit(s.value("RotorMyLon", 0.0).toDouble(), -180.0, 180.0);
        grid->addWidget(lonEdit, 1, 1);

        auto* saveQthBtn = new QPushButton("Save QTH");
        saveQthBtn->setStyleSheet(
            "QPushButton { background: #1a2a3a; border: 1px solid #304050; "
            "border-radius: 3px; color: #c8d8e8; font-size: 11px; padding: 3px 10px; }"
            "QPushButton:hover { background: #203040; }");
        grid->addWidget(saveQthBtn, 2, 1);

        connect(saveQthBtn, &QPushButton::clicked, this, [latEdit, lonEdit] {
            auto& s = AppSettings::instance();
            s.setValue("RotorMyLat", latEdit->text().toDouble());
            s.setValue("RotorMyLon", lonEdit->text().toDouble());
            s.save();
        });
    ...
    // PstRotator group
    {
        auto* group = new QGroupBox("PstRotator Connection");
        group->setStyleSheet(kGroupStyle);
        auto* grid = new QGridLayout(group);
        grid->setSpacing(6);

        grid->addWidget(new QLabel("Host:"), 0, 0);
        auto* hostEdit = new QLineEdit(s.value("RotorHost", "127.0.0.1").toString());
        hostEdit->setStyleSheet(kEditStyle);
        hostEdit->setPlaceholderText("127.0.0.1");
        grid->addWidget(hostEdit, 0, 1);

        grid->addWidget(new QLabel("UDP Port:"), 1, 0);
        auto* portSpin = new QSpinBox;
        portSpin->setRange(1024, 65535);
        portSpin->setValue(s.value("RotorPort", 12000).toInt());
        portSpin->setStyleSheet(
            "QSpinBox { background: #1a2a3a; border: 1px solid #304050; "
            "border-radius: 3px; color: #c8d8e8; font-size: 12px; padding: 2px 4px; }");
        grid->addWidget(portSpin, 1, 1);
        ...
        auto* testBtn = new QPushButton("Test (AZ=0°)");
        auto* resultLabel = new QLabel;
        resultLabel->setStyleSheet("QLabel { color: #00b4d8; font-size: 11px; }");

        for (auto* btn : {saveBtn, testBtn}) {
            btn->setStyleSheet(
                "QPushButton { background: #1a2a3a; border: 1px solid #304050; "
                "border-radius: 3px; color: #c8d8e8; font-size: 11px; padding: 3px 10px; }"
                "QPushButton:hover { background: #203040; }");
        ...
        btnRow->addWidget(resultLabel);
        btnRow->addStretch();
        grid->addLayout(btnRow, 2, 0, 1, 2);

        connect(saveBtn, &QPushButton::clicked, this, [hostEdit, portSpin] {
            auto& s = AppSettings::instance();
            s.setValue("RotorHost", hostEdit->text().trimmed());
            s.setValue("RotorPort", portSpin->value());
            s.save();
        });

        connect(testBtn, &QPushButton::clicked, this, [hostEdit, portSpin, resultLabel] {
            RotorController rc;
            rc.setTarget(hostEdit->text().trimmed(),
                         static_cast<quint16>(portSpin->value()));
            bool ok = true;
            QObject::connect(&rc, &RotorController::errorOccurred, &rc,
                             [&ok](const QString&) { ok = false; });
            rc.sendAzimuth(0.0);
            resultLabel->setText(ok ? "Sent AZ=0°" : "Send failed");
            resultLabel->setStyleSheet(
                ok ? "QLabel { color: #00e060; font-size: 11px; }"
    ...
    auto* note = new QLabel(
        "PstRotator listens on UDP (default port 12000). "
        "Right-click any spot in the Spot List tab to point the antenna.");
    note->setWordWrap(true);
    note->setStyleSheet("QLabel { color: #607080; font-size: 11px; padding: 8px; }");
    vbox->addWidget(note);
