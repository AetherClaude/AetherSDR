...
QWidget* RadioSetupDialog::buildPeripheralsTab()
{
    ...
    // discoveredIpFn (optional): returns a device IP learned out-of-band
    // (e.g. from the radio's amplifier status) to pre-fill when no saved IP
    // exists and no live connection is active (#1039).
    // outStatusLabel (optional): receives a pointer to the status QLabel so
    // the caller can update it with connection-error text (#1039).
    auto buildRow = [&](int row, const QString& label, const QString& ipKey,
                        ...
                        std::function<QString()> discoveredIpFn = {},
                        QLabel** outStatusLabel = nullptr) {
        // Device label
        auto* devLbl = new QLabel(label);
        devLbl->setStyleSheet(kLabelStyle);
        grid->addWidget(devLbl, row, 0);

        // IP field — pre-fill from settings, live connection, or radio-discovered IP
        ...
        } else if (discoveredIpFn) {
            // Use the radio-discovered IP so the user can see and confirm it (#1039)
            QString discovered = discoveredIpFn();
            if (!discovered.isEmpty())
                ipEdit->setText(discovered);
        }
    ...
    if (m_tgxl) {
        QLabel* tgxlStatusLbl = nullptr;
        // Pass the radio-discovered TGXL IP as fallback so users can see it
        // even when auto-connect silently fails (#1039)
        auto updateTgxl = buildRow(1, "Tuner Genius XL (TGXL)", "TGXL_ManualIp", "TGXL_ManualPort", 9010,
            ...
            [this]() { return m_model->tunerModel().tgxlIp(); },
            &tgxlStatusLbl);
        connect(m_tgxl, &TgxlConnection::connected, this, updateTgxl);
        connect(m_tgxl, &TgxlConnection::disconnected, this, updateTgxl);
        // Show TCP error reason in the status label so users know why the
        // connection failed rather than seeing a silent "Not connected" (#1039)
        if (tgxlStatusLbl) {
            connect(m_tgxl, &TgxlConnection::connectionFailed, this,
                    [tgxlStatusLbl](const QString& err) {
                tgxlStatusLbl->setText("Error: " + err);
                tgxlStatusLbl->setStyleSheet(
                    "QLabel { color: #e06060; font-size: 11px; }");
            });
        }
    }
